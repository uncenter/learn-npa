...
import {
	...
	isCorrect,
	mergeArrays,
} from '../../jd-app/phonetic-alphabet/src/utils';
...
const AnswerCard = (props: any) => {
	...
	const [isWindowSmall, setIsWindowSmall] = createSignal(window.innerWidth < 768);
	setInterval(() => {
	...
	return (
		<div>
			<div class="rounded-lg shadow-lg mt-8 text-center mx-6 uppercase">
				...
				<div class={`p-6 flex text-lg ${isWindowSmall() ? 'flex-col' : ''}`}>
		...
		<>
			<Dialog.Root open={open()} onOpenChange={setOpen}>
				<Dialog.Trigger asChild>
					<Button.Root class={twMerge('mr-1', styles.button.blue)} onClick={() => setOpen(true)}>
						Lookup
					</Button.Root>
				</Dialog.Trigger>
				<Dialog.Portal>
					...
					<div>
						<Dialog.Content>
							<div>
								<Dialog.Title>NATO/FAA Phonetic Alphabet</Dialog.Title>
								<Dialog.CloseButton>
									<Close />
								</Dialog.CloseButton>
							</div>
...
export default function Quiz() {
	...
	const [bias, setBias] = createSignal(
		localStorage.getItem('bias') ? parseInt(localStorage.getItem('bias') || '') : 2,
	);
	...
	function addCharacters(word: string) {
		...
		localStorage.setItem('pastCharacters', JSON.stringify(data.pastCharacters));
	}
	createEffect(() => {
		if (submitted()) {
			addCharacters(word() || '');
	...
	const [isWindowSmall, setIsWindowSmall] = createSignal(window.innerWidth < 768);
	const [isWindowSuperSmall, setIsWindowSuperSmall] = createSignal(window.innerWidth < 500);
		...
		<>
			...
			<Drawer opened={isOpen()} placement="right" onClose={onClose}>
				...
				<DrawerContent>
					...
					<DrawerBody>
						<div class="flex flex-col mx-4 gap-6">
							<div>
								...
								<CheckboxGroup
									colorScheme="info"
									defaultValue={JSON.parse(localStorage.getItem('wordLists') || '[]')}
									display="flex"
									gap="$4"
									m="auto"
								>
							...
							<div>
										...
										<PopoverContent>
											...
											<PopoverBody>
												<p class="text-sm">
													...
													distribution of characters. Without, words will be selected at random,
													leaning towards words with common characters.
												</p>
											</PopoverBody>
										</PopoverContent>
									</Popover>
								...
								<Select
									defaultValue={bias()}
									onChange={(e) => setBias(e) && localStorage.setItem('bias', e)}
								>
									<SelectTrigger>
										<SelectValue />
										<SelectIcon />
									...
									<SelectContent>
										<SelectListbox>
											...
											<SelectOption value={2}>
												<SelectOptionText>Medium</SelectOptionText>
												<SelectOptionIndicator />
											</SelectOption>
											<SelectOption value={3}>
												<SelectOptionText>High</SelectOptionText>
												<SelectOptionIndicator />
											</SelectOption>
										</SelectListbox>
									</SelectContent>
					...
					<DrawerFooter>
						<Button.Root
							class={twMerge('mr-3 bg-red-500', styles.button.outline.red)}
							onclick={() => {
								localStorage.clear();
						...
						<Button.Root class={twMerge('mr-3', styles.button.green)} onClick={onClose}>
							Save & Close
						</Button.Root>
					</DrawerFooter>
				</DrawerContent>
